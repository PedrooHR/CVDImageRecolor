...
solver::solver(grid *A, dataset *B)
{
  ...
  // System Matrix
  cudaMallocManaged(&Taxons, numPixels * sizeof(int));
...
__global__ void gpuCalcTaxons(float *Datapoints, float *Graph, int *Taxons, int size)
{
  ...
  if (i < size)
  {
    ...
    {
      float localDist = (Datapoints[i * 3 + 1] - Graph[j * 2 + 0]) * (Datapoints[i * 3 + 1] - Graph[j * 2 + 0]);
      localDist += (Datapoints[i * 3 + 2] - Graph[j * 2 + 1]) * (Datapoints[i * 3 + 2] - Graph[j * 2 + 1]);
...
void solver::calcTaxons()
{
  ...
  gpuCalcTaxons<<<div, 1024>>>(Dataset->Datapoints, Grid->Location, Taxons, numPixels);
